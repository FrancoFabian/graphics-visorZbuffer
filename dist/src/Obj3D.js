...
var Obj3D = /** @class */ (function () {
    ...
    Obj3D.prototype.getObj = function () {
        return this;
    };
    Obj3D.prototype.getPositions = function () {
        var positions = [];
        for (var i = 0; i < this.polyList.length; i++) {
            var polygon = this.polyList[i];
            var vertices = polygon.getNrs();
            var polygonPositions = [];
            for (var j = 0; j < vertices.length; j++) {
                var vertex = this.e[Math.abs(vertices[j])];
                polygonPositions.push({ x: vertex.x, y: vertex.y, z: vertex.z });
            }
            positions.push(polygonPositions);
        }
        return positions;
    };
    Obj3D.prototype.setPositions = function (positions) {
        if (positions.length !== this.w.length) {
            throw new Error('El número de posiciones no coincide con el número de vértices');
        }
        for (var i = 0; i < this.w.length; i++) {
            this.w[i] = positions[i] || new Point3D(0, 0, 0);
        }
    };
    return Obj3D;
}());
